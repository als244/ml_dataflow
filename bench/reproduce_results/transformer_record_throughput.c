...
int main(int argc, char * argv[]){
	...
	if (argc != 9 && argc != 10){
		fprintf(stderr, "Error. Usage: ./transformerRecordThroughput <host_mem_gb> <dev_mem_gb> <seqlen: [num tokens]> <model_path> <model size billions> <num_steps> <num_steps_to_skip_for_recording> <output_filepath> [compute_frac]\n");
		return -1;
	}
	...
	for (int i = 0; i < n_layers; i++){
		sys_blocks[i] = init_transformer_block(i, block_dt, compute_dt,
														...
														eps, theta,
														model_dim, num_q_heads, num_kv_heads, head_dim,
														expert_dim,
														moe_config,
														pointer_alignment);
