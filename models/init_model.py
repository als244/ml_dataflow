...
def main(config_path, model_dir_path):
    ...
    if "head_dim" not in config:
        raise ValueError(f"head_dim not found in config")
    ...
    for i in range(n_layers):
        ...
        ## q matrix
        q_proj = torch.empty((attn_dim, model_dim), dtype=attn_dtype).uniform_(-recip_model_sqrt, recip_model_sqrt).view(attn_view_dtype).numpy()

        all_weights.append(q_proj.reshape(-1))

        total_params += attn_dim * model_dim
        active_params += attn_dim * model_dim
        ...
        ## save
        o_proj = torch.empty((model_dim, attn_dim), dtype=attn_dtype).uniform_(-recip_model_sqrt, recip_model_sqrt).view(attn_view_dtype).numpy()

        all_weights.append(o_proj.reshape(-1))

        total_params += model_dim * attn_dim
        active_params += model_dim * attn_dim
