...
from liger_kernel.ops.rope import liger_rotary_pos_emb as LigerRope
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_complex: torch.Tensor):
        ...
        xq, xk = LigerRope(xq, xk, self.cos_vals, self.sin_vals)
...
class DecoderBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.attention_norm = LigerRMSNorm(args.dim, eps=args.norm_eps, dtype=self.dtype)
        self.ffn_norm = LigerRMSNorm(args.dim, eps=args.norm_eps, dtype=self.dtype)
...
class Llama3Model(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.norm = LigerRMSNorm(params.dim, eps=params.norm_eps, dtype=self.dtype)
