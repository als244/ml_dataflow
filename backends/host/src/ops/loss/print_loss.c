...
int print_chunk_loss_host(void * _print_chunk_loss_host_op_args){
    ...
    printf("[LOSS: Step %d, Seq %d, Chunk %d, Num Tokens: %d] Avg Loss: %f\n", step_num, seq_id, chunk_id, num_tokens, avg_loss);
...
int print_step_loss_host(void * _print_step_loss_host_op_args){
    ...
    printf("[LOSS: Step %d, Num Chunks: %d, Total Tokens: %d] Avg Loss: %f\n", step_num, num_chunks, total_tokens, avg_chunk_loss);
