...
float get_seq_flops(int seq_len, int vocab_size, int model_dim, int attn_dim, int kv_dim, int is_causal, int num_shared_experts, int num_total_routed_experts, int num_active_routed_experts, int expert_dim, int num_layers, 
	...
	float attn_dim_f = (float) attn_dim;
	...
	// q
	seq_flops_fwd += 2 * seq_len_f * model_dim_f * attn_dim_f;
	// for k, v
	...
	float fwd_attn_flops = attn_flop_ratio * 2 * 2 * seq_len_f * seq_len_f * attn_dim_f;

	seq_flops_fwd += fwd_attn_flops;
	seq_attn_flops += fwd_attn_flops;

	// attention out projection
	seq_flops_fwd += 2 * seq_len_f * attn_dim_f * model_dim_f;
...
float get_chunk_block_flops(int chunk_size, int prior_seq_len, int max_seq_len, int model_dim, int attn_dim, int kv_dim, int is_causal, int num_shared_experts, int num_total_routed_experts, int num_active_routed_experts, int expert_dim) {
	...
	// q proj
	chunk_block_flops += 2 * chunk_size_f * model_dim_f * attn_dim_f;
	...
	if (prior_seq_len > 0){
		...
		chunk_block_flops += attn_flop_ratio * 2 * 2 * chunk_size_f * new_seq_len_f * attn_dim_f;
	}
	else{
		int seqs_per_chunk = chunk_size / max_seq_len;
		chunk_block_flops += seqs_per_chunk * attn_flop_ratio * 2 * 2 * max_seq_len_f * max_seq_len_f * attn_dim_f;
	}
...
float get_recompute_flops(int num_seqs_per_round, int seq_len, int n_layers, int model_dim, int attn_dim, int is_causal, int num_shared_experts, int num_active_routed_experts, int expert_dim,
	...
	// q proj
	recompute_flops += total_recompute_chunks * (2 * chunk_size_f * model_dim_f * attn_dim_f);

	// attn out proj
	recompute_flops += total_recompute_chunks * (2 * chunk_size_f * attn_dim_f * model_dim_f);
	...
	// We are using flash attention, so we by default recompute the seq_len x seq_len attention score matrix.
	float recompute_attn_flops = num_seqs_per_round_f * n_layers_f * (attn_flop_ratio * 2 * seq_len_f * seq_len_f * attn_dim_f);
	...
	for (int chunk_loc = 0; chunk_loc < num_inp_only_saved; chunk_loc++){
		...
		if (chunk_size > chunk_seq_len){
			...
			recompute_flops += seqs_per_chunk * attn_flop_ratio * 2 * 2 * chunk_seq_len * chunk_seq_len * attn_dim_f;
			recompute_attn_flops += seqs_per_chunk * attn_flop_ratio * 2 * 2 * chunk_seq_len * chunk_seq_len * attn_dim_f;
		}
		else{
			recompute_flops += attn_flop_ratio * 2 * 2 * chunk_size_f * chunk_seq_len * attn_dim_f;
			recompute_attn_flops += attn_flop_ratio * 2 * 2 * chunk_size_f * chunk_seq_len * attn_dim_f;
		}
	}
...
int start_step_metrics(void * _step_throughput_op_args){
	...
	for (int i = 0; i < num_seqs; i++){
		...
		float total_seq_flops = get_seq_flops(seq_len, step_throughput_op_args->vocab_size, step_throughput_op_args->model_dim, step_throughput_op_args->attn_dim, step_throughput_op_args->kv_dim, step_throughput_op_args->is_causal,
