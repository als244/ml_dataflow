            ...
            if (lane_id == 0){
                ...
                for (int j = 0; j < WARP_SIZE; j++){
                    ...
                    if ((warp_cur_vals[warp_id * WARP_SIZE + j] > min_top_k_val) && ((i + j) < n_experts)) {
                        ...
                        min_top_k_ind = 0;
                        uint16_t min_top_k_exp = 0xFFFF;
                        for (int k = 0; k < top_k_experts; k++){
                            // tie to lower index expert so using <=
                            float cur_top_k_val = warp_top_k_expert_vals[warp_id * top_k_experts + k];
                            uint16_t cur_top_k_exp = warp_top_k_expert_inds[warp_id * top_k_experts + k];
                            if ((cur_top_k_val < min_top_k_val) || (cur_top_k_val == min_top_k_val && cur_top_k_exp > min_top_k_exp)) {
                                min_top_k_val = cur_top_k_val;
                                min_top_k_ind = k;
                                min_top_k_exp = cur_top_k_exp;
                            }
                        }
                    }
                }
...
extern "C" __global__ void default_select_experts_bf16_kernel(int total_tokens, int n_experts, int top_k_experts,  __nv_bfloat16 * X_routed, float * token_expert_weights, uint16_t * chosen_experts, int * expert_counts, int * expert_counts_cumsum, int * host_expert_counts) {
    ...
    __nv_bfloat16 * warp_cur_vals = (__nv_bfloat16 *) sdata;


    __nv_bfloat16 * warp_top_k_expert_vals = warp_cur_vals + (num_warps * WARP_SIZE);
    ...
    // running cutoff to determine if needing to search
    // through
    __nv_bfloat16 min_top_k_val;
    uint16_t min_top_k_ind;

    __nv_bfloat16 routed_val;
    __nv_bfloat16 max_val_top_k;
    ...
    for (int i = lane_id; i < top_k_experts; i+=WARP_SIZE) {
        warp_top_k_expert_vals[warp_id * top_k_experts + i] = NEG_INF_DEV_BF16;
        warp_top_k_expert_inds[warp_id * top_k_experts + i] = (uint16_t) i;
    }
    ...
    for (int row_id = row_offset + warp_id; row_id < row_offset + rows_per_block; row_id+=num_warps){
        ...
        max_val_top_k = NEG_INF_DEV_BF16;
        // reset per-token init values...
        min_top_k_val = NEG_INF_DEV_BF16;
        min_top_k_ind = 0;
        ...
        for (int i = 0; i < n_experts; i+=WARP_SIZE){
            ...
            if ((i + lane_id) < n_experts){
                ...
                warp_cur_vals[warp_id * WARP_SIZE + lane_id] = NEG_INF_DEV_BF16;
            }
            ...
            if (lane_id == 0){
                ...
                for (int j = 0; j < WARP_SIZE; j++){
                    ...
                    if ((warp_cur_vals[warp_id * WARP_SIZE + j] > min_top_k_val) && ((i + j) < n_experts)) {
                        ...
                        // reset min top k val
                        min_top_k_val = POS_INF_DEV_BF16;
                        min_top_k_ind = 0;
                        uint16_t min_top_k_exp = 0xFFFF;
                        for (int k = 0; k < top_k_experts; k++){
                            // tie to lower index expert so using <=
                            __nv_bfloat16 cur_top_k_val = warp_top_k_expert_vals[warp_id * top_k_experts + k];
                            uint16_t cur_top_k_exp = warp_top_k_expert_inds[warp_id * top_k_experts + k];
                            if ((cur_top_k_val < min_top_k_val) || (cur_top_k_val == min_top_k_val && cur_top_k_exp > min_top_k_exp)) {
                                min_top_k_val = cur_top_k_val;
                                min_top_k_ind = k;
        ...
        if (lane_id == 0){
            ...
            // can easy parallelize across lanes...
            for (int k = 0; k < top_k_experts; k++){;
                top_k_sum_exp += expf(__bfloat162float(warp_top_k_expert_vals[warp_id * top_k_experts + k] - max_val_top_k));
            }
            ...
            for (int k = 0; k < top_k_experts; k++){
                ...
                // set the gate value
                token_expert_weights[(orig_token_row * (uint64_t) top_k_experts) + k] = expf(__bfloat162float(warp_top_k_expert_vals[warp_id * top_k_experts + k] - max_val_top_k)) / top_k_sum_exp;
