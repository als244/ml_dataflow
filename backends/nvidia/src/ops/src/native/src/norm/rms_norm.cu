...
extern "C" __global__ void default_rms_norm_bf16_kernel(
    ...
    for (int i = thread_id; i < n_cols_div_8; i += blockDim.x) {
        ...
        const float2 f2_0 = __bfloat1622float2(__uint_as_bfloat162(packed_x.x));
        const float2 f2_1 = __bfloat1622float2(__uint_as_bfloat162(packed_x.y));
        const float2 f2_2 = __bfloat1622float2(__uint_as_bfloat162(packed_x.z));
        const float2 f2_3 = __bfloat1622float2(__uint_as_bfloat162(packed_x.w));
    ...
    for (int i = thread_id; i < n_cols_div_8; i += blockDim.x) {
        ...
        const float2 w2_0 = __bfloat1622float2(__uint_as_bfloat162(packed_w.x));
        const float2 w2_1 = __bfloat1622float2(__uint_as_bfloat162(packed_w.y));
        const float2 w2_2 = __bfloat1622float2(__uint_as_bfloat162(packed_w.z));
        const float2 w2_3 = __bfloat1622float2(__uint_as_bfloat162(packed_w.w));
