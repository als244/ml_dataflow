...
The project is built from ~10k lines of C, 8 logically unique (memory-bound) GPU kernels, and wrappers over [Flash Attention](https://github.com/Dao-AILab/flash-attention) and vendor BLAS libraries (performance critical computation kernels). The only depedencies are the backend userspace driver & BLAS lib along with -lcrypto. For Nvidia backend it assumes that libs and headers are in standard location `/usr/local/cuda`. 
