...
class Device:
    ...
    def _initialize_tasks_and_state(self):
        ...
        ## not saving any activations... leaving room for the gradients of last layer
        ## to fill in during the bwd x round
        if self.activations_capacity <= self.total_chunks:
            raise ValueError("Activations capacity must be greater than total chunks")
        else:
            left_over_from_fwd_cutoff = self.activations_capacity - self.total_chunks
            activation_ind_cutoff = max(0, (total_activations - left_over_from_fwd_cutoff))
        ...
        if self.device_id == head_device_id:
            ...
            for i in range(self.chunks_per_seq - 1, self.total_chunks, self.chunks_per_seq):
                for j in range(i, i - self.chunks_per_seq, -1):
                    self.fwd_computation_queue.append((j, self.total_layers, False, False, transfer_direction, self.headFrames))
        ...
        while cur_layer_id >= 0:
            ...
            for i in range(self.chunks_per_seq - 1, self.total_chunks, self.chunks_per_seq):
                for j in range(i, i - self.chunks_per_seq, -1):
                    ...
                    self.bwd_computation_queue.append((j, cur_layer_id, True, False, current_transfer_dir_bX, bwdX_time)) 
    ...
    def handle_completed_transfers(self, T, all_devices):
        ...
        if self.is_inbound_transferring and self.inbound_queue and (self.cur_inbound_start_time + self.cur_inbound_duration <= T):
            ...
            if chunk_id == -1: # Weight or Head state
                ...
                if 0 <= target_idx < len(self.activations_buffer):
                    self.activations_buffer[target_idx] = (0, chunk_id, layer_id, is_grad)
        ...
        if self.is_outbound_transferring and self.outbound_queue and (self.cur_outbound_start_time + self.cur_outbound_duration <= T):
            ...
            if chunk_id >= 0 and not is_only_context: # Activation saved
                try:
                    idx_to_free = self.activations_buffer.index((-2, chunk_id, layer_id, is_grad))
                    self.activations_buffer[idx_to_free] = -1
                    self.activations_empty_slot_ind = self._find_first_free_idx(self.activations_buffer)
                except ValueError:
                    print(f"T={T}, Dev {self.device_id}: WARN - Could not find activation after out complete ({chunk_id},{layer_id}) marked for outbound transfer.")
    ...
    def handle_computation_depends(self, T):
        ...
        if not is_head_task and has_bwd_task:
            ...
            needs_act_buffer_space = False
            if b_bX:
                needs_act_buffer_space = True

            if b_bW:
                fwd_act_key = (0, b_cid, b_lid, False)
                has_fwd_activation = fwd_act_key in self.activations_buffer
                grad_act_key = (0, b_cid, b_lid, True)
                has_grad_activation = grad_act_key in self.activations_buffer

            
            

            has_act_buffer_space = False
            if needs_act_buffer_space and self.activations_empty_slot_ind is not None:
            ...
            if b_bX:
                ...
                has_bwd_deps = has_weight and has_input_transition and has_context and has_outbound_trans_space and has_act_buffer_space
                ...
                if not has_act_buffer_space: bwd_missing_items.append("Congested (Act.)")

                if not has_act_buffer_space and b_cid == 1 and b_lid == 43:
                    print(f"T={T}, Dev {self.device_id}: ERROR - No act buffer space for BwdX C{b_cid},L{b_lid}")
                    print(f"Activations buffer: {self.activations_buffer}")
                    print(f"Activations empty slot index: {self.activations_empty_slot_ind}")
                    
                    
                    
            elif b_bW:
                computation_type_str = "Bwd W"
                has_bwd_deps = has_fwd_activation and has_grad_activation
                if not has_fwd_activation: bwd_missing_items.append("No Fwd Act.")
                if not has_grad_activation: bwd_missing_items.append("No Grad. Act.")
            ...
            if has_bwd_deps:
                ...
                if needs_act_buffer_space:
                    self.activations_buffer[self.activations_empty_slot_ind] = (-2, b_cid, b_lid, True)
                    self.activations_empty_slot_ind = self._find_first_free_idx(self.activations_buffer)
        ...
        if has_fwd_task:
            ...
            if has_fwd_deps:
                ...
                if needs_act_buffer_space:
                    self.activations_buffer[self.activations_empty_slot_ind] = (-2, f_cid, f_lid, False)
                    self.activations_empty_slot_ind = self._find_first_free_idx(self.activations_buffer)
    ...
    def handle_bwd_prefetch_context(self, chunk_id, cur_layer_id, next_layer_id):
        ...
        if (0, chunk_id, next_layer_id, False) in self.activations_buffer:
        ...
        if (-2, chunk_id, next_layer_id, False) in self.activations_buffer:
    ...
    def handle_bwd_prefetch_fwd_act(self):
        if self.activations_stack_next_ind >= 0:
            ...
            self.activations_buffer[next_act_idx] = (-2, cid, lid, False)
            self.activations_empty_slot_ind = self._find_first_free_idx(self.activations_buffer)
    ...
    def handle_computation(self, T):
        ...
        if self.is_computing and (self.cur_computation_start_time + self.cur_computation_duration <= T):
            ...
            if is_fwd:
                ...
                act_idx = self.activations_buffer.index((-2, cid, lid, False))  
                if should_keep_in_buffer:
                    self.activations_buffer[act_idx] = (0, cid, lid, False)
                else:
                    self.outbound_queue.append((cid, lid, False, False, self.savedActivationsFrames))
            elif bX:
                print(f"T={T}, Dev {self.device_id}: Current Task: {self.current_task}")
                print(f"Activations buffer: {self.activations_buffer}")
                print(f"Activations empty slot index: {self.activations_empty_slot_ind}")

                act_idx = self.activations_buffer.index((-2, cid, lid, True))
                self.activations_buffer[act_idx] = (0, cid, lid, True)
            ...
            if is_fwd: # FWD Finished
                ...
                ## set current activations we just used to be -1...
                act_idx_fwd = self.activations_buffer.index((0, cid, lid, False))
                act_idx_grad = self.activations_buffer.index((0, cid, lid, True))
                self.activations_buffer[act_idx_fwd] = -1
                self.activations_buffer[act_idx_grad] = -1
                self.activations_empty_slot_ind = self._find_first_free_idx(self.activations_buffer)
