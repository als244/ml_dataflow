...
#define TO_SAVE_EMBEDDING_BWD 0
...
#define TO_SAVE_HEAD_BWD 0

#define TO_SAVE_BWD_LAYER 0
...
int dataflow_submit_transformer_head(Dataflow_Handle * dataflow_handle, int compute_stream_id,
	...
	if (TO_SAVE_DATA && TO_SAVE_HEAD){
		ret = save_file(dataflow_handle, compute_stream_id, -1, seq_id, chunk_id, false, "x_act_stream_inp", block_input -> X, total_tokens, embedding_size, fwd_dt);
		if (ret){
			fprintf(stderr, "Error: failed to save head x_act_stream_inp file...\n");
			return -1;
		}
	}
...
int dataflow_submit_transformer_moe_block(Dataflow_Handle * dataflow_handle, int compute_stream_id, int secondary_compute_stream_id,
	...
	if (TO_SAVE_DATA && TO_SAVE_LAYER && ((LAYER_ID_TO_SAVE == -1) || (layer_id == LAYER_ID_TO_SAVE))){
		ret = save_file(dataflow_handle, compute_stream_id, layer_id, seq_id, chunk_id, false, "attn_norm_w", transformer_block -> w_attn_norm, model_dim, 1, fwd_dt);
		if (ret){
			fprintf(stderr, "Error: failed to save attention nor file...\n");
			return -1;
		}
	...
	*/
	...
	//int * host_expert_mapping = batch_moe_config -> host_expert_mapping + layer_id * (total_q * top_k_active);
	...
	if (TO_SAVE_DATA && TO_SAVE_LAYER && ((LAYER_ID_TO_SAVE == -1) || (layer_id == LAYER_ID_TO_SAVE))){
		...
		ret = save_file(dataflow_handle, compute_stream_id, layer_id, seq_id, chunk_id, false, "x_selected_experts", working_activations -> chosen_experts, total_q, top_k_active, DATAFLOW_UINT16);
		if (ret){
			fprintf(stderr, "Error: failed to save x_selected_experts file...\n");
			return -1;
		}
		
		ret = save_file(dataflow_handle, compute_stream_id, layer_id, seq_id, chunk_id, false, "x_router_weights", working_activations -> token_expert_weights, total_q, top_k_active, DATAFLOW_FP32);
		if (ret){
			fprintf(stderr, "Error: failed to save x_router_weights file...\n");
			return -1;
		}
	...
	ret = dataflow_submit_default_build_expert_mapping(dataflow_handle, compute_stream_id, 
                                ...
                                working_activations -> expert_mapping, working_activations -> token_mapping);
...
int dataflow_submit_transformer_moe_block_bwd_x(Dataflow_Handle * dataflow_handle, int compute_stream_id, int secondary_compute_stream_id,
	...
	int seen_tokens = set_moe_working_activations_offsets(working_activations, total_q, model_dim, ffn_dim, 
		num_shared_experts, num_routed_experts, host_expert_counts, x_el_bwd_size);

	if (seen_tokens != total_q * top_k_active){
		fprintf(stderr, "Error: seen tokens (%d) != total q (%d) * top k (%d) in bwd_w...\n", seen_tokens, total_q, top_k_active);
		return -1;
	}

	seen_tokens = set_moe_working_activations_offsets(fwd_activations, total_q, model_dim, ffn_dim, 
		num_shared_experts, num_routed_experts, host_expert_counts, x_el_bwd_size);
...
int dataflow_submit_transformer_moe_block_bwd_w(Dataflow_Handle * dataflow_handle, int compute_stream_id, int secondary_compute_stream_id,
	...
	ret = dataflow_submit_default_prepare_experts(dataflow_handle, compute_stream_id, 
								fwd_dt, fwd_dt,
								total_q, model_dim, top_k_active, bwd_activations -> recomputed_activations -> recomputed_ffn_norm,
								fwd_activations -> token_mapping, 
								expert_zones);
	if (ret){
		fprintf(stderr, "Error: failed to submit prepare experts...\n");
	...
	for (int i = 0; i < num_routed_experts; i++){
		...
		ret = dataflow_submit_matmul(dataflow_handle, cur_compute_stream,
								...
								1.0, 1.0,  // Accumulate gradients
								cur_expert_zone, (bwd_activations -> x_1)[num_shared_experts + i], (grad_weights -> w_1)[num_shared_experts + i], (grad_weights -> w_1)[num_shared_experts + i],
								streamKernelWorkspaceBytes, curKernelWorkspace);
		if (ret) {
			fprintf(stderr, "Error: failed to submit w1 weight gradient computation...\n");
			return -1;
		...
		ret = dataflow_submit_matmul(dataflow_handle, cur_compute_stream,
								...
								model_dim, cur_expert_num_tokens, ffn_dim, 
								1.0, 1.0,  // Accumulate gradients
								cur_expert_zone, (bwd_activations -> x_3)[num_shared_experts + i], (grad_weights -> w_3)[num_shared_experts + i], (grad_weights -> w_3)[num_shared_experts + i],
								streamKernelWorkspaceBytes, curKernelWorkspace);
		if (ret) {
			fprintf(stderr, "Error: failed to submit w3 weight gradient computation...\n");
			return -1;
